...
def holdKey(key,durantion):
    ...
    for letter in input:
        win32api.keybd_event(letter,win32api.MapVirtualKey(letter,0),2,0) #释放按键
        sleep(np.abs((rand()-0.5)/10)+0.2)
...
def get_keybd_input(key):
    return win32api.GetAsyncKeyState(vk_keys(key))


def vk_keys(key):
    VK_CODE = { 'left_mouse':0x01,
           'right_mouse':0x02,
           'backspace':0x08,
           'tab':0x09,
           'clear':0x0C,
