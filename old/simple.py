...
def PressKey(hexKeyCode):
    extra = c_ulong(0)
    ui_ = Union_Input()
    ui_.ki = KeyBdInput( 0, hexKeyCode, KEYEVENTF_SCANCODE, 0, pointer(extra) )
    x = Input( c_ulong(1), ui_ )
    windll.user32.SendInput(1, pointer(x), sizeof(x))

def ReleaseKey(hexKeyCode):
    extra = c_ulong(0)
    ui_ = Union_Input()
    ui_.ki = KeyBdInput( 0, hexKeyCode, KEYEVENTF_SCANCODE | KEYEVENTF_KEYUP, 0, pointer(extra) )
    x = Input( c_ulong(1), ui_ )
    windll.user32.SendInput(1, pointer(x), sizeof(x))
...
while (time.time() - now < 5):
    ...
    #PressKey(0x1E) # press F
    
    #ReleaseKey(0x1E) #release F
    PressKey(0x1E)
    time.sleep(.5)
    ReleaseKey(0x1E)
